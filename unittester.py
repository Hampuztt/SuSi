...
from iris_majority import find_nearest_class_by_distance
...
class TestImputePredictionWithDistance(unittest.TestCase):
    def test_imputation_with_direct_neighbors(self):
        ...
        result = find_nearest_class_by_distance(grid, np.array([1, 1]))
        self.assertEqual(
    ...
    def test_imputation_with_further_neighbors(self):
        ...
        result = find_nearest_class_by_distance(grid, np.array([0, 0]))
        self.assertEqual(
            result,
            3,
    ...
    def test_assertion_without_any_neighbors(self):
        ...
        with self.assertRaises(AssertionError):
            find_nearest_class_by_distance(grid, np.array([0, 0]))
